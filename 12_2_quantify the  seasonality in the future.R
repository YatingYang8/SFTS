...
dd_month <- Temp_proj_fitted_result %>%
  #filter(year >2023) %>%
...
dd_month$total_cases_copy = dd_month$total_cases

dd_month$date <- as.Date(paste0(dd_month$year,"-",dd_month$month, "-01"))

head(dd_month)

season_data1 <- dd_month %>%
  mutate(total_cases = ifelse(year <= 2023, total_cases, fitted.values))

season_data2 <- dd_month %>%
  mutate(total_cases = ifelse(year <= 2023, total_cases, fitted.values_lower))

season_data3 <- dd_month %>%
  mutate(total_cases = fitted.values)

dd_month <- season_data1 %>%
  group_by(year) %>%
  mutate(
    cumulative_cases = cumsum(total_cases),
    total_cases_year = sum(total_cases, na.rm = TRUE),
    cumulative_ratio = cumulative_cases / total_cases_year
  ) %>%
  ungroup()

# 找到每年的高峰月份（病例最多的月份）
peak_months <- dd_month %>%
  group_by(year) %>%
  filter(total_cases == max(total_cases)) %>%
  summarise(peak_month = month[which.max(total_cases)], .groups = "drop")

### 保留具有相同高峰病例数的月份
# peak_months <- dd_month %>%
#   group_by(year) %>%
#   filter(total_cases == max(total_cases)) %>%  
#   summarise(peak_month = month, .groups = "drop")

# 找到高流行月份（累计病例占比 > 90%）
prevalence_months <- dd_month %>%
  group_by(year) %>%
  filter(cumulative_ratio >= 0.05 & cumulative_ratio <= 0.95) %>%
  summarise(
...
seasonality_data <- left_join(peak_months, prevalence_months) %>%
  mutate(
    start_month = ifelse(peak_month < start_month, peak_month, start_month),
    end_month = ifelse(peak_month > end_month, peak_month, end_month)
  )
...
p1 <- ggplot(seasonality_data, aes(y = year)) +
  # 绘制月份范围
  geom_linerange(aes(xmin = start_month, xmax = end_month), color = "gray50", size = 1) +
  # 绘制 peak_month 位置的方块
  geom_point(aes(x = peak_month), color = "red3", size = 4, shape = 15) +
  # 设置坐标轴
  #scale_x_continuous(breaks = 1:12, labels = month.name, limits = c(0, 13)) +
  scale_x_continuous(
    breaks = 1:12,
    labels = month_labels,
    limits = c(0.5, 12.5),
  ...
  scale_y_reverse(breaks = seq(2011, 2028, by = 1)) +  # 反转 y 轴的顺序，使年份从上到下显示
  # 设置标签和标题
  labs(
    x = "Month",
    y = "Year",
    title = "Seasonality of SFTS",
    subtitle = "2011-2023: Observed data, 2024-2028: Predicted data"
  ) +
  # 使用经典主题并调整元素大小和间距
  theme_classic(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 12),  # x轴标签倾斜
    axis.text.y = element_text(size = 12),  # y轴标签字体大小
    axis.title.x = element_text(size = 14,face = "bold"),  # x轴标题字体大小
    axis.title.y = element_text(size = 14,face = "bold"),  # y轴标题字体大小
    plot.title = element_text(hjust = 0.5, size = 18, face = "bold"),  # 标题居中并加粗
    plot.subtitle = element_text(size = 8, hjust = 0.5),
    plot.margin = unit(c(1, 1, 1, 1), "cm")  # 增加图形边距，使图形更清晰
  )+
  # 添加数据范围说明
  annotate("text", x = 13, y = 2018, label = "Duration", 
           hjust = 1, vjust = -1, size = 4, color = "gray50") +
  annotate("text", x = 13, y = 2017, label = "Peak month", 
           hjust = 1, vjust = -1, size = 4, color = "red3")

p1

#ggsave(p1, file="./output/figures/Figure8_seasonality.pdf", device="pdf", units="in", width=8, height=10)
...
p2 <- ggplot(county_num, aes(x = year, y = counties_above_1)) +
  ...
  labs(
    ...
    subtitle = "2011-2023: Observed data, 2024-2028: Predicted data",  # 可以添加副标题来提供更多信息
    x = "Year", 
  ...
  theme(
    ...
    plot.title = element_text(size = 18, face = "bold", hjust = 0.5),  # 标题字体和居中
    plot.subtitle = element_text(size = 8, hjust = 0.5),  # 副标题字体居中
...
p = gridExtra::grid.arrange(p2,p3,ncol=1,heights=c("1.2","1"))

pp = gridExtra::grid.arrange(p1,p,nrow=1, widths=c("1","1") )

#ggsave(pp, file="./codes for figures/new figures 2025.1.27/Figure6_combined.pdf", device="pdf", units="in", width=15, height=10)
